...
//Set(row int, v vector.Vec) sets a matrix row to the specified vector
...
func (m Mat) Set(row int, v vector.Vec) {
	dim := m.Dim()
	if dim < len(v) {
		vector.LogVecError(v, "Set(row int, v vector.Vec) FAILED vector length less the matrix dim")
		return
	}
	idx := Map(row, 0, dim)
	for i := 0; i < 3; i++ {
...
//Get obtains the Row vector.Vector from the parameter row copies vector value into a
...
//Crossvector.Vec crosses a matrix with a vector by summing the Matrix Row (Row Major)
//With the associative column vector. Assuming matrices are all MxM in this LIBRARY
//vector.Vector is returned length M
func (a Mat) CrossVec(b vector.Vec) vector.Vec {

	if a.Dim() != len(b) {
		vector.LogVecError(b, "Cross vector.Vec Matrix Dim != vector.Vec Dim")
		return vector.VecN(a.Dim())
	}
	r := vector.VecN(a.Dim())
