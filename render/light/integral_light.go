...
func NewRectLight(color mgl.Vec, lum float32, pos mgl.Vec, width float32,
	...
	plane[2][0] = -x1
	plane[2][1] = y1
	myRect := RectLight{&Area{pos, mgl.Vec{}, math.Pi / 2, Lux{color, lum}}, lum, pos, width, height, float32(num_w), float32(num_h),
		math.Pi / 4, plane}
	return &myRect
}
...
func (light *RectLight) Lights() []Area {
	...
	for i := 0; i < int(light.Num_w); i++ {
		for j := 0; j < int(light.Num_h); j++ {
			...
			lgt := Area{nPos, n, cutoff, Lux{light.SampleLx.Rgb(), lum}}
