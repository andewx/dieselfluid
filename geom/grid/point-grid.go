...
func BuildGrid(Scale V.Vec, TransOrigin V.Vec, DimVec V.Vec) Grid {
	MyGrid := Grid{}
	MyGrid.Origin = TransOrigin
	MyGrid.MinXYZ = V.Vec{float32(-1.0), float32(-1.0), float32(-1.0)}
	MyGrid.MinXYZ = V.VecScl(MyGrid.MinXYZ, Scale)
	MinDouble := V.VecScl(MyGrid.MinXYZ, V.Vec{-2.0, -2.0, -2.0})
	LenSides := V.Sub(MyGrid.MinXYZ, MinDouble)
	InvDimVec := V.Vec{1 / DimVec[0], 1 / DimVec[1], 1 / DimVec[2]}
	MyGrid.StepXYZ = V.VecScl(LenSides, InvDimVec)
...
func BuildKernGrid(Scale V.Vec, TransOrigin V.Vec, kern float32) (Grid, int) {
	MyGrid := Grid{}
	MyGrid.Origin = TransOrigin
	MyGrid.MinXYZ = V.Vec{float32(-1.0), float32(-1.0), float32(-1.0)}
	MyGrid.MinXYZ = V.VecScl(MyGrid.MinXYZ, Scale)
...
func (g Grid) GridPosition(i int, j int, k int) V.Vec {
	p := ijk2Vec(i, j, k)
	return V.Add(g.MinXYZ, V.VecScl(g.StepXYZ, p))
}

//Updates the Internal Grid Component
func (g Grid) UpdateGrid(Scale V.Vec, TransOrigin V.Vec, DimVec V.Vec) Grid {
	//Scale XYZ Components - Regarding Previous Origin
	g.MinXYZ = V.Sub(g.MinXYZ, g.Origin)
	g.MinXYZ = V.VecScl(g.MinXYZ, Scale)
	MinDouble := V.VecScl(g.MinXYZ, V.Vec{-2.0, -2.0, -2.0})
	LenSides := V.Sub(g.MinXYZ, MinDouble)
	InvDimVec := V.Vec{1 / DimVec[0], 1 / DimVec[1], 1 / DimVec[2]}
	g.StepXYZ = V.VecScl(LenSides, InvDimVec)
